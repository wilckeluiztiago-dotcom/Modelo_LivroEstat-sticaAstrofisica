...
$$
\rho_{\mathrm{fino}}(R,z) = \rho_0\,\Sigma(R)\,\operatorname{sech}^{2}\!\left(\frac{z}{2z_{d}}\right)
$$

onde o perfil de superfície radial é quebrado em $R_{\mathrm{break}}$:

$$
\Sigma(R) =
\begin{cases}
\exp\!\left(-\dfrac{R}{R_{d,\mathrm{in}}}\right) & R \le R_{\mathrm{break}} \\[10pt]
\exp\!\left(-\dfrac{R_{\mathrm{break}}}{R_{d,\mathrm{in}}}\right)\exp\!\left(-\dfrac{R-R_{\mathrm{break}}}{R_{d,\mathrm{out}}}\right) & R > R_{\mathrm{break}}
\end{cases}
$$

Valores adotados (calibrados em literatura Gaia/APOGEE para [Fe/H] ≈ 0):

- $R_{d,\mathrm{in}} = 2{,}2\,\mathrm{kpc}$
- $R_{d,\mathrm{out}} = 3{,}5\,\mathrm{kpc}$
- $R_{\mathrm{break}} = 8{,}5\,\mathrm{kpc}$
...
$$
\rho_{\mathrm{espesso}}(R,z) = f_{\mathrm{esp}}\,\rho_0\exp\!\left(-\frac{R}{R_{d,\mathrm{esp}}}\right)\operatorname{sech}^{2}\!\left(\frac{z}{2z_{d,\mathrm{esp}}}\right)
$$

com $f_{\mathrm{esp}} \approx 0{,}07$, $R_{d,\mathrm{esp}} = 2{,}8\,\mathrm{kpc}$, $z_{d,\mathrm{esp}} = 0{,}85\,\mathrm{kpc}$.

### 2.3 Densidade total

$$
\rho_{\mathrm{total}}(R,z) = \rho_{\mathrm{fino}}(R,z) + \rho_{\mathrm{espesso}}(R,z)
$$
...
Proposta uniforme no volume cilíndrico → aceitação com probabilidade proporcional a $\rho_{\mathrm{total}}$:

$$
u \sim \mathcal{U}(0,\rho_{\max}),\qquad \text{aceita se } u < \rho_{\mathrm{total}}(R,z)
$$
...
$$
\pi(\theta) \propto \exp\!\Bigl(-\tfrac{1}{2}\sum_i\bigl(\tfrac{\theta_i - \mu_i}{\sigma_i}\bigr)^{2}\Bigr)
...
- Posteriors MCMC de $R_{d,\mathrm{in}}$, $R_{d,\mathrm{out}}$ e $z_d$.
