...
$$
P(f)=\frac{1}{2}\left[\frac{\left(\sum_j y_j\cos\omega(t_j-\tau)\right)^2}{\sum_j\cos^2\omega(t_j-\tau)}+\frac{\left(\sum_j y_j\sin\omega(t_j-\tau)\right)^2}{\sum_j\sin^2\omega(t_j-\tau)}\right]
$$
...
$$
k(t,t') = A^{2} \exp\left( -\Gamma \sin^{2} \left( \frac{\pi |t-t'|}{P_{\mathrm{rot}}} \right) - \left( \frac{|t-t'|}{\lambda} \right)^{2} \right) + \sigma_{w}^{2} \delta_{tt'}
$$
